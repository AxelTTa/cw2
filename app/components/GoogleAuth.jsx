...
export default function GoogleAuth({ onAuthSuccess, onAuthError }) {
  ...
  const handleGoogleSignIn = () => {
    ...
    googleAuthUrl.searchParams.append('state', generateState())
      ...
      setError('Failed to setup OAuth')
      setIsLoading(false)
    }
  }
  ...
  return (
    <div style={{
      ...
      <button
        ...
        onDoubleClick={handleGoogleSignInPopup}
        disabled={isLoading}
        style={{
          display: 'flex',
          alignItems: 'center',
      ...
      >
        {isLoading ? (
          ...
          <>
            {!GOOGLE_CLIENT_ID && (
              <span style={{ color: '#d93025', fontSize: '12px' }}>⚠️</span>
            )}
            ...
            {GOOGLE_CLIENT_ID ? 'Sign in with Google' : 'OAuth Not Configured'}
          </>
        )}
      </button>
